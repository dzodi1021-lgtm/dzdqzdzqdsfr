...
function buildCardPage({
  ...
  const marqueeText = escapeHtml(titleMarqueeText || '');
...
<body>
  <main class="card" role="main">
    ...
    ${enableTitleMarquee && marqueeText ? `
      <div class="marqueeWrap" aria-hidden="true">
        <div class="marquee">
          <span>${marqueeText}</span><span>${marqueeText}</span><span>${marqueeText}</span><span>${marqueeText}</span>
        </div>
      </div>
    ` : ``}
  ...
  <script>
    (function () {
      var base = ${JSON.stringify(titleMarqueeText || '')};
      if (!base) return;
      var pad = '   •   ';
      var text = base + pad;
      var i = 0;
...
function buildExistsPage() {
  return buildCardPage({
    ...
    subtitle: 'This script endpoint is up.',
    chips: ['Script endpoint', 'Non-raw view'],
    hintHtml: "This page only confirms the script exists. Raw content is meant to be used by a Lua client via <code>?raw=1</code>.",
    accent: '#60a5fa',
  });
}
...
function buildNotFoundPage() {
  return buildCardPage({
    title: 'Not found',
    subtitle: 'No script matched that name.',
    chips: ['404', 'scripts/'],
    hintHtml: 'Double-check the requested name and the file inside <code>scripts/</code> (<code>.lua</code> / <code>.txt</code>).',
    accent: '#fbbf24',
  });
}

function buildRawBlockedPage() {
  const imgUrl = 'https://i.postimg.cc/Jzm7phVG/image.png';
  const iconHtml = `<img src="${imgUrl}" alt="epstein" />`;
  const marquee = 'STOP TRYING TO SKID';
  return buildCardPage({
    title: 'Access denied',
    subtitle: 'Keep trying to see the code and im gonna put you on the files',
    chips: ['Browser blocked', 'Raw endpoint'],
    hintHtml: 'This endpoint does not serve raw content to browsers.',
    accent: '#fb7185',
    iconHtml,
